...
export default function Content({
  ...
  setConversationsArr: Dispatch<SetStateAction<any[]>>;
  ...
  const [messages, setMessages] = useState<Message[]>(
    conversationsArr[selectedConversationId] ?? []
  );
  ...
  // Set up the messages when by the selected conversation
  useEffect(() => {
    setMessages(conversationsArr[selectedConversationId] ?? []);
  }, [selectedConversationId]);

  // update the messages on the related conversation to save them
  useEffect(() => {
    const copyConversationsArr = structuredClone(conversationsArr);
    copyConversationsArr[selectedConversationId] = [...messages];
    setConversationsArr(copyConversationsArr);
  }, [messages]);
  ...
  const editMessageLlmResponse = (ind: number, responseContent: string) => {
    const clonedMessages = structuredClone(messages);
    clonedMessages[ind].content = responseContent;
  ...
  return (
    <div>
      {messages?.length > 0 && (
        <div className="flex flex-1 flex-col p-4 sm:ml-64">
            ...
            <div key={`message-${index}`} className="flex flex-col md:flex-row">
              <div className="w-full md:w-3/4 md:pr-2">
                ...
                <LLMResponseComponent
                  ...
                  // isToEditLlmResponse={isToEditLlmResponse}
                  editLlmResponse={editMessageLlmResponse}
                />
                {message.followUp && (
